...
document.getElementById('btn-run').addEventListener('click', async function() {
    ...
    consoleLine(con, 'c-dim', '$ sha512 "' + msg + '"');
    consoleLine(con, '', '');

    try {
        var res = await fetch('/api/sha512', {
            method: 'POST',
        ...
        consoleLine(con, '', '');
        consoleLine(con, 'c-res', 'SHA-512 = ' + data.digest);
        consoleLine(con, '', '');

        title.textContent = 'sha-512 output  [' + data.digest.substring(0,16) + '...]';
        status.className = 'status';
